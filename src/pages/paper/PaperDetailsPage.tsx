...
const PaperDetailPage = () => {
	...
	const [data, setData] = useState<PapersDataType | null>(null);

	useEffect(() => {
		(async () => {
			if (id == undefined) return;
			try {
				const res = await ContentService.getDetailPaper(id);
	...
	const onFavoriteClick = async () => {
		...
		const res = await ContentService.setFavorite(id, !checked, FAVORITEENUM.PAPER);
