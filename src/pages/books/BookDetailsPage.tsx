...
const BookDetailPage = () => {
	...
	const [data, setData] = useState<BookDataType | null>(null);

	useEffect(() => {
		(async () => {
			if (id == undefined) return;
			try {
				const res = await ContentService.getDetailBook(id);
	...
	const onFavoriteClick = async () => {
		...
		const res = await ContentService.setFavorite(id, !checked, FAVORITEENUM.BOOK);
		...
		<div className="text-white">
			<div className="mb-4 flex flex-col-reverse flex-wrap sm:mb-0 sm:flex-row sm:flex-nowrap">
				...
				<div>
					...
					<Separator title="Writer" value={data?.writer} />
					<Separator title="Edition" value={data?.bookEdition.toString()} />
					<Separator title="Semester" value={data?.semester} />
					<Separator title="Price" value={'₹' + data?.price} />
					<Separator title="Pages" value={data?.pages.toString()} />
